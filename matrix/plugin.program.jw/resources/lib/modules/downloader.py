...
class Downloader:
    ...
    def download_build(self, name,zippath,meth='session', stream=True):
        ...
        dp.create(name + ' - ' + str(length2) + ' MB', 'downloading virus...')
        dp.update(0, 'downloading virus...')
        cancelled = False
        ...
        if length:
            ...
            if meth in ['session', 'requests']:
                for chunk in response.iter_content(chunk_size=max(1000000,500000)):
                    ...
                    dp.update(perc, 'downloading virus...' + '\n' + str(size2) + '/' + str(length2) + 'MB')
                    if dp.iscanceled():
                        cancelled = True
                        break
            elif meth in 'urllib':
            ...
            dp.update(50, 'downloading virus...')
        ...
        if cancelled:
            ...
            return
        if length:
            dp.update(100, 'downloading virus...done!' + '\n' + str(size2) + '/' + str(length2) + 'MB')
        else:
            dp.update(100, 'downloading virus...done!')
        tempzip.close()
